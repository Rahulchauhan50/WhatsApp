import { EndCall, setIncomingVoiceCall, setVoiceCall, setAddMessages } from "@/redux/features/userSlice";
...
function IncomingCall({ socket }) {
  const { incomingVoiceCall, UserInfo } = useSelector((state) => state.user);
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed inset-0 z-50 bg-[#0b141a] flex flex-col items-center text-white">
        <div className="flex flex-col items-center mt-20 gap-2">
          <div className="relative w-28 h-28 rounded-full overflow-hidden border-4 border-teal-light">
            <Image
              src={incomingVoiceCall.profileImage || "/default_avatar.png"}
              alt={incomingVoiceCall.name}
              fill
              className="object-cover"
            />
          </div>
          <h2 className="text-2xl font-light mt-4">{incomingVoiceCall.name}</h2>
      ...
      <div className="hidden md:flex fixed bottom-6 right-6 z-50 w-80 rounded-2xl bg-[#1f2c34] text-white shadow-2xl border border-[#2a3942] overflow-hidden animate-slideUp">
        <div className="flex items-center gap-4 p-4 w-full">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium truncate">{incomingVoiceCall.name}</p>
            <p className="text-xs text-gray-400">Incoming voice call</p>
