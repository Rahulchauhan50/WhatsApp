...
function CallMessage({ message }: CallMessageProps) {
  const { CurrentChatUser, UserInfo } = useSelector((state: any) => state.user);
  ...
  const getCallLabel = () => {
    ...
    const dur = callData.duration ? formatDuration(callData.duration) : "";
