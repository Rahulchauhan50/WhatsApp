...
function ChatContainer({ socket }: ChatContainerProps) {
  const { Messages, CurrentChatUser, UserInfo, pagination, isLoadingOlderMessages } = useSelector((state: any) => state.user)
  ...
  useEffect(()=>{
    if(socket?.current) {
      ...
      socket.current.on("msg-read", (data: any) => {
          ...
          <div className="flex flex-col justify-end w-full gap-2">
            {
              Object.entries(Messages || {}).map(([date, messageList])=>{
                return (
                  ...
                  {(messageList as any[])?.map((msg, index ) => (
